...
function meetingRoutines(uiType) {
  ...
  waitForElement(document, "#webclient").then(() => {
    ...
    iframe.addEventListener("load", () => {
      ...
      const iframeDOM = document.querySelector("#webclient").contentDocument
...
function transcriptMutationCallback(mutationsList) {
  mutationsList.forEach((mutation) => {
    ...
    if (currentPerson.childNodes.length > 0) {
      const currentPersonName = currentPerson.firstChild.textContent
      const currentTranscriptText = currentPerson.lastChild.textContent

      // let currentPersonName, currentTranscriptText
      // if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
      //   // console.log('A child node was added.')
      //   const currentPerson = mutation.addedNodes[0]

      //   currentPersonName = currentPerson.firstChild.textContent
      //   currentTranscriptText = currentPerson.lastChild.textContent

      // } else if (mutation.type === 'characterData') {
      //   // console.log('The text content of a child node changed.')

      //   const currentPerson = mutation.target.parentElement.parentElement
      ...
      //   // Update buffers for next mutation by faking a new person
      //   personNameBuffer = ""
...
function getLastTranscriptTextOfPerson(personName) {
  for (let i = transcript.length - 1; i >= 0; i--) {
    if (transcript[i].personName === personName) {
      console.log("Returning" + transcript[i].transcriptText + "of" + personName)
      return (transcript[i].transcriptText)
    }
  }
